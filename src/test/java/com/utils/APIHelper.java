    ...
    public Response makeRequest(HTTPVerb httpVerb, String endpoint, Object... headerAndBody) {
        ...
        if (body == null) {
            if (HTTPVerb.GET == httpVerb) {
                response = given().headers(new Headers(requestHeader)).get(endpoint);
            } else if (HTTPVerb.POST == httpVerb) {
                response = given().headers(new Headers(requestHeader)).post(endpoint);
            } else if (HTTPVerb.PUT == httpVerb) {
                response = given().headers(new Headers(requestHeader)).put(endpoint);
            } else if (HTTPVerb.DELETE == httpVerb) {
                response = given().headers(new Headers(requestHeader)).delete(endpoint);
            }
        } else {
            if (HTTPVerb.GET == httpVerb) {
